...
async function isSlugTaken(
  ...
  const index = await kv.get(`beat-slug:${slug}`);
  if (!index) return false;
  if (excludeBeatId && index.beatId === excludeBeatId) return false;
  return true;
}
...
async function generateUniqueSlug(
  ...
  let slug = base;
  let counter = 2;
  while (await isSlugTaken(slug, excludeBeatId)) {
    slug = `${base}-${counter}`;
    counter++;
  }
  return slug;
...
async function setBeatSlugIndex(slug: string, beatId: string) {
  await kv.set(`beat-slug:${slug}`, { beatId });
}

async function removeBeatSlugIndex(slug?: string) {
  if (slug) {
    await kv.del(`beat-slug:${slug}`);
  }
}

async function backfillBeatSlugs() {
  try {
    ...
    for (const beat of beats || []) {
      ...
      if (!beat.slug) {
        const slug = await generateUniqueSlug(beat.title || "beat");
        beat.slug = slug;
        await kv.set(`beat:${beat.id}`, beat);
        await setBeatSlugIndex(slug, beat.id);
      } else {
        const index = await kv.get(`beat-slug:${beat.slug}`);
        if (!index) {
          await setBeatSlugIndex(beat.slug, beat.id);
        }
      }
    }
  } catch (error) {
    console.error("Error backfilling beat slugs:", error);
  }
}
  ...
  try {
    ...
    if (!existingBeats || existingBeats.length === 0) {
      ...
      for (const beat of defaultBeats) {
        const id = generateId();
        const slug = await generateUniqueSlug(beat.title);
        ...
        await setBeatSlugIndex(slug, id);
      }
...
// Get beat by slug (direct sales link)
app.get("/make-server-fe24c337/beats/slug/:slug", async (c) => {
  try {
    ...
    const beat = await kv.get(`beat:${index.beatId}`);
    if (!beat) {
      return c.json({ error: "Beat not found" }, 404);
    }
    ...
    console.error("Error fetching beat by slug:", error);
    return c.json({ error: "Failed to fetch beat" }, 500);
  }
});
...
app.get("/make-server-fe24c337/beats", async (c) => {
  try {
    let beats = await kv.getByPrefix("beat:");

    // Ensure all beats have slugs - backfill on fetch
    if (beats && beats.length > 0) {
      for (const beat of beats) {
        if (beat && beat.id && !beat.slug) {
          const slug = await generateUniqueSlug(beat.title || "beat", beat.id);
          beat.slug = slug;
          await kv.set(`beat:${beat.id}`, beat);
          await setBeatSlugIndex(slug, beat.id);
          console.log(`✅ Generated slug for beat "${beat.title}": ${slug}`);
        }
      }
    }
...
app.post("/make-server-fe24c337/beats", async (c) => {
  try {
    ...
    const id = generateId();
    const slug = await generateUniqueSlug(beatData.title || "beat");
    const beat = {
      id,
      ...beatData,
      slug,
...
app.delete("/make-server-fe24c337/beats/:id", async (c) => {
  try {
    ...
    await removeBeatSlugIndex(existingBeat?.slug);
...
app.put("/make-server-fe24c337/beats/:id", async (c) => {
  try {
    ...
    let slug = existingBeat?.slug;
    if (!slug) {
      slug = await generateUniqueSlug(
        beatData.title || existingBeat?.title || "beat",
        id,
      );
    }
...
app.post("/make-server-fe24c337/create-checkout", async (c) => {
  try {
    const { beatId, beatTitle, price, email, beatSlug } =
      await c.req.json();
    ...
    const successUrl = `${origin}${returnPath}?payment=success&beat=${beatId}&session={CHECKOUT_SESSION_ID}`;
